...
- (void)done:(id)sender
{
    if ([self.delegate respondsToSelector:@selector(cropViewController:didFinishCroppingImage:)]) {
        ...
        NSString *imgName = [NSString stringWithFormat:@"%d.png",arc4random() % 1000000];
        NSData *imageData = UIImageJPEGRepresentation(self.cropView.croppedImage, 0.5);
        // 获取沙盒目录
        
        _fullPath = [[NSHomeDirectory() stringByAppendingPathComponent:@"Documents"] stringByAppendingPathComponent:imgName];
        
        // 将图片写入文件 保存图片至沙盒
        [imageData writeToFile:_fullPath atomically:NO];
        
        
        [self.delegate cropViewController:self didFinishCroppingImage:_fullPath];
